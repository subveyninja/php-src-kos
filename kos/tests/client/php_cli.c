...
static int is_phpt_file(char *name) {
...
static void list_files_recursively(char *basePath)
{
    struct dirent *dp;
    DIR *dir = opendir(basePath);

    if (!dir) {
        fprintf(stderr, "Error while opendir(): %s, path is: %s\n", strerror(errno), basePath);
        return;
    }
    ...
    while (1) {
        ...
        if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0) {
            ...
            if (strcmp(basePath, TESTS_PATH) != 0) {
                strcat(path, "/");
            }
            strcat(path, dp->d_name);

            if (is_phpt_file(dp->d_name)) {
                g_argv[g_argc++] = strdup(path);
            }
...
# endif
    ...
    list_files_recursively(TESTS_PATH);

    // To avoid recursive runs
    if (argc == 1) {
        argv = g_argv;
        argc = g_argc;
    }
...
out:
    ...
    if (0 > cli_sock) {
        ...
        ssize_t send_res = send(cli_sock, "End", 3, 0);
        if (0 > send_res) {
