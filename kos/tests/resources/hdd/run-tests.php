...
function main(): void
{
    ...
    for ($i = 1; $i < $argc; $i++) {
        ...
        if (!$is_switch) {
            $selected_tests = true;
            // KOS: we can't call realpath() too much
            // coz stat() or lstat() break testing after ~8k calling.
            // So only absolute paths for `.phpt` files
            // $testfile = realpath($argv[$i]);
            if ($argv[$i][0] != "/") {
                die('Only absolute paths' . PHP_EOL);
            }
            $testfile = $argv[$i];
            ...
            if (!$testfile && strpos($argv[$i], '*') !== false && function_exists('glob')) {
                ...
                if (0 && is_dir($testfile)) {
                    // KOS: we can't call is_dir() too much
                    // coz stat() or lstat() break testing after ~8k calling.
                    // So only a list of `.phpt can use
                    // find_files($testfile);
                    die("KOS: you can't run directories, only a list of `.phpt`" . PHP_EOL);
                } else {
    ...
    // KOS: for searching exection bin
    $php = "Cli_that_not_exist";
...
";
	...
	if (array_key_exists('ENV', $section_text)) {
        show_result('SKIP', $tested, $tested_file, "reason: KOS.ENV test requiring ENV section");
        junit_init_suite(junit_get_suitename_for($shortname));
        junit_mark_test_as('SKIP', $shortname, $tested, 0, 'reason: KOS.ENV test requiring ENV section');
        return 'SKIPPED';
    }
